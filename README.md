...
**Compartment-specificity check:** real MBONs were classified by which real dopamine neuron cluster dominates their input — PAM (reward-associated) vs PPL1 (punishment-associated), per Aso et al. 2014. Restricting the plasticity rule to only PPL-dominant MBONs and leaving PAM-dominant MBONs untouched reproduces a genuine, non-trivial prediction: PPL-dominant MBONs declined 7.4% while PAM-dominant MBONs stayed flat (-0.1%) — **despite both living in the same fully recurrent whole-brain network**, where the effect could have leaked across compartments but didn't. See [`Drosophila_brain_model/learn_compartment_specific.py`](Drosophila_brain_model/learn_compartment_specific.py).
