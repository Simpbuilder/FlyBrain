"""
Fly obstacle-dodge game, started/stopped externally (Claude watches the
control/game doc on the dashboard and starts/kills this process - the
process itself has no network access, so it can't poll the doc directly).

Runs bounded batches of lives (never truly endless - always has a real
stopping point), recording every life's full trajectory for the game
viewer AND, on every collision, exporting a real brain snapshot (which
real neurons were active right at the moment of failure, including the
real thermosensory "pain" pathway) to ../failure_snapshot.json, which
Claude pushes to the live dashboard's brain_live/current doc.
"""
import json
import time
import numpy as np
...
def pain_and_capture(episode, ticks):
    """Fire the real thermosensory pathway (failure signal) and export a
    real brain snapshot of that moment for the live dashboard."""
    neu.v = params['v_0']
    neu.g = 0 * mV
    stim.rate = 0 * Hz
...
for episode in range(1, N_LIVES + 1):
    ...
    while True:
        ...
        if obs_dist <= 0:
            if fly_lane == obs_lane:
                pain_and_capture(episode, ticks)
                break
            obs_lane = int(rng.integers(0, N_LANES))
            obs_dist = 5
        if ticks > 400:
    ...
    if time.time() - last_stats_export > 15:
        last_stats_export = time.time()
        with open('../game_state.json', 'w') as f:
            json.dump({'episode': episode, 'last_survival': ticks, 'avg_survival': round(survival_avg, 2),
                       ...
                       'elapsed_s': round(time.time() - t0, 1), 'done': False}, f)
...
with open('../game_state.json', 'w') as f:
    json.dump({'episode': N_LIVES, 'last_survival': lives[-1]['ticks'], 'avg_survival': round(survival_avg, 2),
               'best_survival': int(best_survival), 'weights': w.tolist(),
               'elapsed_s': round(time.time() - t0, 1), 'done': True}, f)
print('>>> wrote fly_game_batch.json and final game_state.json', flush=True)
