...
def process(path, label):
    ...
    # density grid for everything NOT individually highlighted
    bg_mask = ~np.isin(cat, ['odor_A_kc', 'mbon'])
    ...
    # exact spikes for the highlighted populations (odor-A KCs, MBONs)
    hi_mask = np.isin(cat, ['odor_A_kc', 'mbon'])
    ...
    out = {
        'grid': grid_list,
        'grid_w': GRID_W, 'grid_h': GRID_H, 'n_tbins': N_TBINS, 't_max': T_MAX,
        'x_min': X_MIN, 'x_max': X_MAX, 'y_min': Y_MIN, 'y_max': Y_MAX,
        'neurons': neurons,
        'spikes': spikes,
        'n_bg_spikes': int(bg_mask.sum()),
        'n_hi_spikes': int(hi_mask.sum()),
    }
    print(f'{label}: {len(neurons)} highlighted neurons, {len(spikes)} highlighted spikes, {int(bg_mask.sum())} background spikes')
    return out


before = process('../spatial_before.json', 'before')
after = process('../spatial_after.json', 'after')

# static anatomical silhouette from ALL traced neuron positions (not just
# the ones that spiked), so the real brain shape shows as a backdrop
import pandas as pd
...
sx = pos_df['pos_x'].to_numpy()
sy = pos_df['pos_y'].to_numpy()
SIL_W, SIL_H = 160, 100
...
print('silhouette from', len(sx), 'neuron positions, max cell count', sil_grid.max())

with open('../spatial_viz.json', 'w') as f:
    json.dump({
...
import os
print('output size (KB):', os.path.getsize('../spatial_viz.json') / 1024)
